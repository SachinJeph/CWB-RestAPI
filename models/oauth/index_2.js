...
var OAuthClient = require('./oauth_client'); //db.OAuthClient;
...
function getClient(clientId, clientSecret, cb){
	...
	OAuthClient.findOne(options).then(function(client){
		if (!client) cb(new Error("client not found"));
...
function grantTypeAllowed(clientId, grantType, cb){
	...
	if(grantType === 'password') cb(null, true);
	else cb(new Error('Not Valid Grant Type'), false);
};
...
function saveAccessToken(token, client, expires, user, cb){
	console.log("saveToken", token, client, expires, user);
	return Promise.all([
...
module.exports = {
 ...
 grantTypeAllowed: grantTypeAllowed,
  ...
  saveAccessToken: saveAccessToken,//saveOAuthAccessToken, renamed to
